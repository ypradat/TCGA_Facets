...
if config["params"]["civic"]["run_per_sample"]["cna"]:
    ...
    rule somatic_cna_civic_preprocess:
        ...
        shell:
            """
            python -u workflow/scripts/03.1_civic_preprocess.py \
                --table_alt {input.cna} \
                --table_cln {input.cln} \
                --table_gen {input.gen} \
                --gen_gene_name name \
...
rule somatic_cna_civic_filter:
    ...
    shell:
        """
        python -u workflow/scripts/03.2_civic_postprocess.py \
            --input {input} \
            --output {output} &> {log}
        """
...
if config["params"]["oncokb"]["run_per_sample"]["cna"]:
    ...
    rule somatic_cna_oncokb_preprocess:
        ...
        shell:
            """
            python -u workflow/scripts/03.3_oncokb_preprocess.py \
                --table_alt {input.cnv} \
                --table_cln {input.cln} \
                --table_gen {input.gen} \
...
rule somatic_cna_oncokb_filter:
    ...
    shell:
        """
        python -u workflow/scripts/03.4_oncokb_postprocess.py \
            --input {input.cna} \
            --rules {input.rul} \
            --alteration_type {params.alteration_type} \
            --output {output} &> {log}
